...
<script>
  $(document).ready( function () {
    ...
    <title>Thunderzone-Mylist</title>
  </head>
  <body>
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark" id="nv">
    ...
    <form class="d-flex searchl" style="margin-bottom: 5%;"  method="POST" >
      <input class="form-control me-2 " type="search" #id="sno" name="sno" placeholder="Remove Game from your list just enter serial number here" aria-label="Search">
    ...
    $sql=" DELETE FROM `favourite`  WHERE sno='$sno'";
    $result=mysqli_query($conn,$sql);
    
  }
      ?>
    </form>
    <h1 style="text-align:centre;margin-left: 40%; color:aqua;">My Games</h1>
  ...
  //  $category=$_POST[`category`];
   $sql="SELECT * FROM `game_data`,favourite  WHERE game_data.sno=favourite.sno";
   $result=mysqli_query($conn,$sql);
